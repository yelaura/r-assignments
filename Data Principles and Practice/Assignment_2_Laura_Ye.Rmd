...
```{r, echo=TRUE, comment=NA, warning=FALSE, message=FALSE, cache=TRUE}
library(tidyverse)
...
user_searches_time <- user_searches %>%
  ...
  number_clicks = as.numeric(!is.na(click_url))
...
Number of queries during a certain time of day

```{r, echo=TRUE, comment=NA, warning=FALSE, message=FALSE}
ggplot(user_searches_time, aes(query_time_hour)) + 
  geom_bar() + 
  geom_line(aes(query_time_hour,click_url)) +
