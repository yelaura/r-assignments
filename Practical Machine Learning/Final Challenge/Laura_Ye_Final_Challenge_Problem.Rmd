...
Categorical variables with more than 2 levels like DPLURAL will be converted to dummy variables using one-hot-encoding. Categorical variables with only 2 levels will be converted to ones and zeros.

```{r}
# convert true/false variables to 1,0s
# birth_data_clean <- birth_data %>%
#   mutate(GESTREC3 = as.integer(ifelse(GESTREC3 == ">= 37 weeks", 1, 0)),
#          CIG_REC = as.integer(CIG_REC),
#          ULD_MECO = as.integer(ULD_MECO),
#          ULD_PRECIP = as.integer(ULD_PRECIP),
#          ULD_BREECH = as.integer(ULD_BREECH),
#          URF_DIAB = as.integer(URF_DIAB),
#          URF_CHYPER = as.integer(URF_CHYPER),
#          URF_PHYPER = as.integer(URF_PHYPER), 
#          URF_ECLAM = as.integer(URF_ECLAM),
#          atRisk = as.integer(atRisk))
...
The data is now ready to be split into a train (70%), and test (30%) set. The train set is what we will use to train models. Finally the test set is what we will use to select the best tuned model and also the best model overall.

```{r}
set.seed(3)
assignment <- sample(1:3, size = nrow(birth_data_clean), prob = c(0.7,0.15,0.15), replace = TRUE)

train <- birth_data_clean[assignment == 1, ]
valid <- birth_data_clean[assignment == 2, ]
test <- birth_data_clean[assignment == 3, ]
...
Here is a summary of the two model performances so far:
...
Fit a *univariate* OLSR (Ordinary Least Squares Regression) model, adhering to OLSR
assumptions, predicting **income from prestige** only
